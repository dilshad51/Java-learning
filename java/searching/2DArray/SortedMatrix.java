...
class SortedMatrix{
    public static void main(String[] args) {
        ...
        if(arr.length==0) System.out.println("Invalid");
    ...
    static int[] search(int [][]matrix, int target){
        ...
        if(matrix[rStart][0]<target && target<=matrix[rStart][matrix[0].length-1]){
           return binarySearch(matrix, rStart, 0, cols-1, target);
        }
        else return binarySearch(matrix, rStart+1, 0, cols-1, target);
    }
}